...
class complex_Linear(torch.nn.Module):
    def __init__(self, num_inputs, num_outputs, bias=False):
        super(complex_Linear, self).__init__()
        self.linear_r = torch.nn.Linear(num_inputs, num_outputs, bias=bias)
        self.linear_i = torch.nn.Linear(num_inputs, num_outputs, bias=bias)

    def forward(self, xr, xi):
        o_r = self.linear_r(xr) - self.linear_i(xi)
        o_i = self.linear_r(xi) + self.linear_i(xr)
        return o_r, o_i

# Complex GRU class. MH, LMS 2020.
class complex_GRU(torch.nn.Module):
    def __init__(self, num_input, num_hidden, num_layers, bias=False):

        super(complex_GRU, self).__init__()

        self.gru_r = torch.nn.GRU(input_size=num_input, hidden_size=num_hidden, num_layers=num_layers, batch_first=True, bias=bias)
        self.gru_i = torch.nn.GRU(input_size=num_input, hidden_size=num_hidden, num_layers=num_layers, batch_first=True, bias=bias)

    def forward(self, xr, xi):
        GRU_rr, _ = self.gru_r(xr)
        GRU_ri, _ = self.gru_i(xr)
        GRU_ii, _ = self.gru_r(xi)
        GRU_ir, _ = self.gru_i(xi)
        xr = GRU_rr - GRU_ii
        xi = GRU_ir + GRU_ri
        return xr, xi
...
# Wrapper for Complex Conv. Module. (pass in a normal conv2d torch module)
# Example:
#       ComplexConvWrapper(nn.ConvTranspose2d, conv_cfg, bias=False)
#       conv_cfg includes: kernel size, stride, number of output channels passed as keywords!
#
...
# Internal class: do not use on it is own !
...
# Initialize as a normal LeakyReLU !
...
# Includes fixes for memory leakage issues in the original implementation: MH LMS 2020
class ComplexBatchNorm(torch.nn.Module):
    ...
    def forward(self, xr, xi):
        ...
        if training:
            ...
            if self.track_running_stats:
                with torch.no_grad(): # Attention: This fix is needed to prevent memory leakage. MH LMS 2020
...
class ComplexBatchNorm_phaseConsistent(torch.nn.Module):
    def __init__(self, num_features, eps=1e-5, momentum=0.1, affine=True,
            track_running_stats=True):
        super(ComplexBatchNorm_phaseConsistent, self).__init__()
        self.num_features        = num_features
        self.eps                 = eps
        self.momentum            = momentum
        self.affine              = affine
    ...
    def forward(self, xr, xi):
        ...
        normFactor = torch.div(1,( Vrr + Vii))
        ...
        yr = (torch.sqrt(normFactor) * xr)
        yi = (torch.sqrt(normFactor) * xi)
